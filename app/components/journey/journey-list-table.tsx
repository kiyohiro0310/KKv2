...
const JourneyListTable = ({ title, lists, isLearning }: { title: string; lists: {}[]; isLearning: boolean;}) => {
  return (
    <div className="w-[80%] md:w-1/2 mx-auto space-y-10 py-6">
      ...
      {lists.map((item: any, index: number) => {
        return (
          <Link href={isLearning ? `/learning/${item._id}` : `/daily/${item._id}` } key={index}>
            <div
              className="w-full flex flex-col md:flex-row justify-start items-center space-x-6 overflow-hidden border-b-[0.5px] border-gray-300 py-4 hover:text-sub transition-all duration-200"
            >
