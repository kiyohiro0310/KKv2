...
const Hero = () => {
  ...
  useEffect(() => {
    if (!contentRef.current || !imageRef.current || !backgroundRef.current)
      return;
    ...
    <div className="relative w-full h-screen flex flex-col-reverse md:flex-row-reverse items-center justify-center text-center md:text-left px-6 overflow-hidden">
      ...
      <div ref={contentRef} className="md:w-1/2">
        ...
        <p className="my-6 text-sm md:text-base lg:text-lg max-w-2xl text-gray-500 dark:text-gray-400">
          Have a blast on your journey—it&apos;s the one adventure that&apos;s
          all yours!
        </p>
        <Link
          href={"/contact"}
        ...
        <div className="flex w-full pt-12 justify-center sm:justify-start">
          <Link
            href={
      ...
      <div
        ref={backgroundRef}
        className="hidden sm:block absolute -right-[5vw] -bottom-[13vw] rounded-[30%] w-[30vw] h-[30vw] bg-sub rotate-45"
      ></div>
      <div
        ref={backgroundRef}
        className="sm:hidden block absolute -right-[5vw] -bottom-[13vw] rounded-[30%] w-[200px] h-[200px] bg-sub rotate-45"
      ></div>
    </div>
  );
};
